...
3. Read open_positions.json — check for any open trades
...
## TRADING PAIRS
Only: BTC/USDT, ETH/USDT, SOL/USDT Never trade low-liquidity altcoins. No trading: 12:00 AM to 5:30 AM IST

## CAPITAL
₹10,000 virtual (PAPER MODE) Track balance in: paper_balance.json

## MODE
PAPER TRADING — fetch REAL live prices from Delta Exchange public API every 10 seconds. Simulate execution at exact real-time market price. Never use fake or estimated prices. Switch to LIVE only when user says exactly: "ACTIVATE LIVE MODE"
...
REGIME C — VOLATILE: BB width > 3x normal OR volume spike > 200% → PAUSE all entries, protect open positions with tight stops Always log which regime detected.
...
Exit:
Take Profit +4% with 1.5% trailing stop | Stop Loss: ATR x 1.5 dynamic
