...
When you receive a heartbeat poll (message matches the configured heartbeat prompt), don't just reply `HEARTBEAT_OK`. Use heartbeats productively! ...

... (rest of original content unchanged) ...

## NEWS & EXTERNAL DATA RULES:
- News is used ONLY to filter trades — never to initiate them.
- If major news is found for BTC/ETH/SOL in the next 30 minutes, skip that signal.
- Never act on news alone without technical indicators confirming.
